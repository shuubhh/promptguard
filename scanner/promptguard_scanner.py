...
# ---------------------------------------------------------------------------
# Secret patterns — ported 1:1 from extension/patterns.js so the scanner's
# findings match what the extension would flag at runtime.
# ---------------------------------------------------------------------------
...
SECRET_PATTERNS = [
    ...
    ("db_connection_string", "Database Connection String", "critical", r"(?:mongodb|postgresql|mysql)://[^\s\"']{10,}"),
    ("private_key", "Private Key (PEM)", "critical", r"-----BEGIN (?:RSA |EC )?PRIVATE KEY-----"),
    ...
    for entry in SECRET_PATTERNS:
        ...
        for m in regex.finditer(text):
            ...
            result.secrets.append({
                ...
                "severity": severity,
                "file": rel_path,
                "preview": matched[:30],  # never store full secret values
            })
